...
object PairingCoordinator {
    ...
    fun configureAndPair(
        ...
        // 2. Perform network pairing handshake with ESP32 controller first
        CoroutineScope(Dispatchers.IO).launch {
            ...
            withContext(Dispatchers.Main) {
                if (result is PairingResult.Success) {
                    val finalMac = if (result.mac.isNotBlank()) result.mac else trimmedMac
                    ...
                    if (!persisted) {
                        onComplete(PairingResult.ConfigurationError("Failed to commit confirmed configuration to secure storage."))
                        return@withContext
                    }
