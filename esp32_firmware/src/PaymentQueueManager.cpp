...
static void clearQueueSlotLocked(int index) {
...
bool canPerformRebootOrOta() {
    if (!isMaintenanceMode()) {
        Serial.println("[PAY QUEUE] Reboot blocked: admission is not closed (maintenance mode off).");
        return false;
    }
    if (hasUnpersistedPayments()) {
        Serial.println("[PAY QUEUE] Reboot blocked: unpersisted transactions remain in RAM.");
        return false;
    }
    if (isCoinSlotArmed() || isRelayHardwareActive()) {
        Serial.println("[PAY QUEUE] Reboot blocked: relay or coin slot hardware is powered/ARMED.");
        return false;
    }
    ...
    if (activeDev.length() > 0 || state == CoinSlotState::ARMED || state == CoinSlotState::DRAINING || state == CoinSlotState::RESERVED_ARMING) {
        Serial.println("[PAY QUEUE] Reboot blocked: active or draining session owner exists.");
        return false;
    }
    if (isrUniversalPulseCount > 0) {
        Serial.println("[PAY QUEUE] Reboot blocked: in-flight coin pulses pending in ISR buffer.");
        return false;
    }
    if (getSessionAccumulatedPulses() > 0) {
        Serial.println("[PAY QUEUE] Reboot blocked: accumulated coin pulses in buffer.");
        return false;
    }
    return true;
}
...
static bool acknowledgeMatchingPayment(const String& txId, const String* sessionId,
    ...
    clearQueueSlotLocked(foundIndex);
    unlockQueue();
...
bool acknowledgePhonePayment(
    ...
    for (int i = 0; i < MAX_PAYMENT_QUEUE_SIZE; i++) {
        if (paymentSlotUsed[i] && 
            ...
            if (recordTarget == deviceId || getDeviceIdFromIp(recordTarget) == deviceId) {
                foundIndex = i;
                break;
            }
        }
